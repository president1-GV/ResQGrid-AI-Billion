...
export const RoleCommandCenter: React.FC<RoleCommandCenterProps> = ({
  ...
  return (
    <div className={`rounded-2xl border shadow-xl transition-all ${
      ...
      <div className={`p-4 border-b flex flex-col md:flex-row md:items-center justify-between gap-3 ${
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className={`flex flex-wrap items-center gap-1.5 p-1 rounded-xl border ${
            isDarkMode ? 'bg-slate-950/60 border-slate-800' : 'bg-slate-100 border-slate-200 shadow-inner'
          }`}>
            <button
        ...
        <div className="flex items-center space-x-2 shrink-0">
          <button
            ...
            className={`flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-xs font-mono font-bold border transition cursor-pointer ${
              isDarkMode
                ? 'bg-slate-800 hover:bg-slate-700 text-sky-300 border-slate-700'
                : 'bg-slate-100 hover:bg-slate-200 text-sky-800 border-slate-300 shadow-sm'
            }`}
          >
      ...
      <div className={`px-4 py-2 border-b text-xs font-mono flex flex-col sm:flex-row sm:items-center justify-between gap-2 ${
        isDarkMode
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className={isDarkMode ? 'text-amber-300/90' : 'text-amber-950 font-medium'}>{MANDATORY_DISCLAIMER}</span>
        </div>
        <span className={`text-[10px] font-mono shrink-0 ${isDarkMode ? 'text-slate-400' : 'text-slate-600 font-medium'}`}>
        ...
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className={`p-4 rounded-xl border space-y-3 ${
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-lg border text-xs transition cursor-pointer flex items-center justify-between group ${
                    isDarkMode
                      ? 'border-red-500/30 bg-red-950/30 hover:bg-red-900/40 text-white'
                      : 'border-red-200 bg-red-50/70 hover:bg-red-100 text-slate-900 shadow-sm'
                  }`}
                >
                  <div>
                    <div className={`font-bold flex items-center gap-1.5 ${isDarkMode ? 'text-red-300' : 'text-red-800 font-bold'}`}>
                  ...
                  <ChevronRight className="w-4 h-4 text-red-500 group-hover:translate-x-0.5 transition" />
                </button>
                ...
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-lg border text-xs transition cursor-pointer flex items-center justify-between group ${
                    isDarkMode
                      ? 'border-amber-500/30 bg-amber-950/30 hover:bg-amber-900/40 text-white'
                      : 'border-amber-200 bg-amber-50/70 hover:bg-amber-100 text-slate-900 shadow-sm'
                  }`}
                >
                  <div>
                    <div className={`font-bold flex items-center gap-1.5 ${isDarkMode ? 'text-amber-300' : 'text-amber-800 font-bold'}`}>
                ...
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-lg border text-xs transition cursor-pointer flex items-center justify-between group ${
                    isDarkMode
                      ? 'border-purple-500/30 bg-purple-950/30 hover:bg-purple-900/40 text-white'
                      : 'border-purple-200 bg-purple-50/70 hover:bg-purple-100 text-slate-900 shadow-sm'
                  }`}
                >
                  <div>
                    <div className={`font-bold flex items-center gap-1.5 ${isDarkMode ? 'text-purple-300' : 'text-purple-800 font-bold'}`}>
                  ...
                  <ChevronRight className="w-4 h-4 text-purple-500 group-hover:translate-x-0.5 transition" />
                </button>
              </div>
              ...
              {cascadeResult && (
                <div className={`p-2.5 rounded-lg border text-xs space-y-1 ${
                  ...
                  <div className={`font-bold flex items-center gap-1 ${isDarkMode ? 'text-emerald-300' : 'text-emerald-900'}`}>
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                    Cascade Shock Solved ({cascadeResult.reallocated_dispatches_count} dispatches re-allocated)
                  </div>
                  <div className={`text-[10px] ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                    Severed Corridors: {cascadeResult.severed_roads?.join(', ') || 'None'}
                  </div>
                  <div className={`text-[10px] line-clamp-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            ...
            <div className={`p-4 rounded-xl border space-y-3 lg:col-span-2 ${
              ...
              {pendingAllocations.length === 0 ? (
                <div className={`p-8 text-center text-xs font-mono rounded-lg ${
                  isDarkMode
                  ...
                  {pendingAllocations.slice(0, 4).map((alc) => (
                    ...
                    >
                      <div className="space-y-0.5">
                        <div className="flex items-center space-x-2">
                          <span className={`font-mono font-bold ${isDarkMode ? 'text-sky-400' : 'text-sky-700'}`}>{alc.id}</span>
                          <span className={`font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-900'}`}>{alc.resource_type.toUpperCase()}</span>
                        ...
                        <div className={`text-[11px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                          Origin: <span className={`${isDarkMode ? 'text-slate-300' : 'text-slate-800'} font-semibold`}>{alc.source_warehouse_id}</span> &rarr; Destination: <span className={`${isDarkMode ? 'text-slate-300' : 'text-slate-800'} font-semibold`}>{alc.destination_zone_id}</span>
                        </div>
                      </div>
        ...
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className={`p-4 rounded-xl border ${
              ...
              <div className={`text-xs font-mono uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-600 font-semibold'}`}>National NDRF Readiness</div>
              <div className={`text-2xl font-bold mt-1 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>12 Battalions</div>
              <div className={`text-[10px] mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>3 Battalions pre-staged in active theater</div>
            </div>

            <div className={`p-4 rounded-xl border ${
              isDarkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-slate-50 border-slate-200'
            }`}>
              <div className={`text-xs font-mono uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-600 font-semibold'}`}>Critical Vulnerability Wards</div>
              <div className={`text-2xl font-bold mt-1 ${isDarkMode ? 'text-red-400' : 'text-red-700'}`}>{criticalZones.length} Sectors</div>
              <div className={`text-[10px] mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Priority Triage Index &ge; 80 / 100</div>
            </div>

            <div className={`p-4 rounded-xl border ${
              isDarkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-slate-50 border-slate-200'
            }`}>
              <div className={`text-xs font-mono uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-600 font-semibold'}`}>Humanitarian Equity Lift</div>
              <div className={`text-2xl font-bold mt-1 ${isDarkMode ? 'text-sky-400' : 'text-sky-700'}`}>+24.8%</div>
              <div className={`text-[10px] mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Over unconstrained shortest-path baseline</div>
            </div>

            <div className={`p-4 rounded-xl border ${
              isDarkMode ? 'bg-slate-950/80 border-slate-800' : 'bg-slate-50 border-slate-200'
            }`}>
              <div className={`text-xs font-mono uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-600 font-semibold'}`}>Supply Deficit Alerts</div>
              <div className={`text-2xl font-bold mt-1 ${isDarkMode ? 'text-amber-400' : 'text-amber-700'}`}>0 Deficits</div>
              <div className={`text-[10px] mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>All sector demands fulfilled by MIP solver</div>
            </div>
          </div>
          ...
          <div className={`p-4 rounded-xl border ${
            ...
            <h3 className={`text-xs font-bold uppercase tracking-wider mb-3 ${isDarkMode ? 'text-slate-300' : 'text-slate-800'}`}>
                ...
                <div
                  ...
                  className={`p-3 rounded-lg border text-xs space-y-1.5 ${
                    zone.priority_score >= 80
                      ? (isDarkMode ? 'border-red-500/40 bg-red-950/20' : 'border-red-300 bg-red-50')
                      : isDarkMode ? 'border-slate-800 bg-slate-900/60' : 'border-slate-200 bg-white'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-900'}`}>{zone.name}</span>
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-mono font-bold ${
                      zone.priority_score >= 80
                        ? (isDarkMode ? 'bg-red-500/30 text-red-300' : 'bg-red-100 text-red-800 border border-red-300')
                        : (isDarkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-200 text-slate-800')
                    }`}>
                      TRIAGE {zone.priority_score.toFixed(1)}
                    </span>
                  </div>
                  <div className={`text-[11px] flex items-center justify-between ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                    <span>Pop: {zone.population.toLocaleString()}</span>
                    <span>Vulnerability: {(zone.vulnerability * 100).toFixed(0)}%</span>
                  </div>
                  <div className={`text-[10px] font-mono pt-1 border-t ${isDarkMode ? 'text-sky-400 border-slate-800' : 'text-sky-700 border-slate-200'}`}>
        ...
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {state.warehouses.map((wh) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className={`font-bold text-sm ${isDarkMode ? 'text-slate-200' : 'text-slate-900'}`}>{wh.name}</div>
                ...
                <div className={`space-y-1.5 pt-2 border-t text-xs ${isDarkMode ? 'border-slate-800/80' : 'border-slate-200'}`}>
                  <div className="flex justify-between">
                    <span className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Drinking Water:</span>
                    <span className={`font-mono font-bold ${isDarkMode ? 'text-sky-300' : 'text-sky-700'}`}>{(wh.inventory?.water ?? 0).toLocaleString()} L</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Emergency Food:</span>
                    <span className={`font-mono font-bold ${isDarkMode ? 'text-amber-300' : 'text-amber-800'}`}>{(wh.inventory?.food ?? 0).toLocaleString()} pkts</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Medical Trauma Kits:</span>
                    <span className={`font-mono font-bold ${isDarkMode ? 'text-emerald-300' : 'text-emerald-800'}`}>{(wh.inventory?.medical_kits ?? 0).toLocaleString()} kits</span>
                  </div>
                </div>

                <div className={`text-[10px] font-mono pt-1 ${isDarkMode ? 'text-slate-500' : 'text-slate-600'}`}>
        ...
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {hazardsSummary.map((h) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className={`font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-900'}`}>{h.name}</span>
                  <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded border ${
                    h.truth_class === 'LIVE' ? (isDarkMode ? 'bg-red-500/20 text-red-300 border-red-500/30' : 'bg-red-100 text-red-900 border-red-300') :
                    h.truth_class === 'NEAR_REAL_TIME' ? (isDarkMode ? 'bg-amber-500/20 text-amber-300 border-amber-500/30' : 'bg-amber-100 text-amber-900 border-amber-300') :
                ...
                <div className={`text-[11px] ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                  Authority: <span className={`${isDarkMode ? 'text-slate-300' : 'text-slate-800 font-semibold'}`}>{h.authority}</span>
                </div>
                <div className={`text-[11px] font-mono ${isDarkMode ? 'text-sky-400' : 'text-sky-700'}`}>
                  Metric: {h.latest_metric}
                </div>
                <div className={`flex items-center justify-between pt-2 border-t text-[10px] font-mono ${
                  isDarkMode ? 'border-slate-800' : 'border-slate-200'
                }`}>
                  <span className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Status: {h.status}</span>
                  <span className={`font-bold ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>{h.threat_level}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeRole === 'field' && (
        <div className="p-5 space-y-4">
          <div className={`p-4 rounded-xl border space-y-2 text-xs ${
            ...
            <h3 className={`font-bold uppercase tracking-wider text-xs ${isDarkMode ? 'text-slate-200' : 'text-slate-900'}`}>
              ...
              {state.roads.map((road) => (
                <div
                  ...
                  className={`p-2.5 rounded-lg border flex items-center justify-between text-xs ${
                    road.status === 'blocked'
                      ? (isDarkMode ? 'bg-red-950/30 border-red-500/40 text-red-300' : 'bg-red-50 border-red-300 text-red-900')
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`max-w-3xl w-full max-h-[80vh] overflow-y-auto rounded-2xl border shadow-2xl p-5 space-y-4 ${
            ...
            <div className={`flex items-center justify-between border-b pb-3 ${
              ...
              <div>
                <span className={`text-xs font-mono font-bold ${isDarkMode ? 'text-sky-400' : 'text-sky-700'}`}>DATA/CATALOG.YAML REGISTRY</span>
                <h3 className={`text-base font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Authoritative Dataset Catalog & SHA-256 Manifests</h3>
              </div>
              <button
            ...
            <div className={`p-3 rounded-lg border text-xs font-mono ${
              isDarkMode ? 'bg-amber-950/40 border-amber-500/30 text-amber-300' : 'bg-amber-50 border-amber-300 text-amber-900'
            }`}>
            ...
            <div className="space-y-3">
              {catalogData?.datasets?.map((ds: any) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className={`font-bold ${isDarkMode ? 'text-slate-100' : 'text-slate-900'}`}>{ds.name}</span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${
                      ds.truth_class === 'LIVE' ? (isDarkMode ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-red-100 text-red-900 border border-red-300') :
                      ds.truth_class === 'SYNTHETIC' ? (isDarkMode ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-amber-100 text-amber-900 border border-amber-300') :
                      (isDarkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-200 text-slate-800')
                    }`}>
                      {ds.truth_class}
                    </span>
                  </div>
                  <div className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>{ds.description}</div>
                  <div className={`grid grid-cols-2 gap-2 text-[10px] font-mono pt-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                    <div>Provider: <span className={`${isDarkMode ? 'text-slate-300' : 'text-slate-800 font-medium'}`}>{ds.provider}</span></div>
                    <div>Cadence: <span className={`${isDarkMode ? 'text-slate-300' : 'text-slate-800 font-medium'}`}>{ds.update_cadence}</span></div>
                    <div className="col-span-2 truncate">SHA-256: <span className={`${isDarkMode ? 'text-sky-400' : 'text-sky-700'}`}>{ds.checksum_sha256}</span></div>
                  </div>
                </div>
              ))}
            </div>
